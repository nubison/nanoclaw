...
function resolveEnvValue(value: string): string {
  // $(command) — run shell command and use stdout (trimmed)
  const withCmd = value.replace(/\$\((.+?)\)/g, (_, cmd) => {
    try {
  ...
  // ${VAR} — substitute host environment variable
  return withCmd.replace(/\$\{(\w+)\}/g, (_, name) => process.env[name] || '');
}
